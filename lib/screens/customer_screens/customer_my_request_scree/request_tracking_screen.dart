...
class _RequestTrackingScreenState
    ...
    if (workerId != null && workerId.isNotEmpty) {
      if (status == 'accepted') {
        return _activeWorkerView(
          ...
          heroSubtitle:
              'You are now connected with a professional.',
          accent: _success,
          showMap: false,
        );
      }
      ...
      if (status == 'on_the_way') {
        return _activeWorkerView(
          ...
          heroSubtitle:
              'Track the worker’s live location and stay connected.',
      ...
      if (status == 'in_progress') {
        return _activeWorkerView(
          ...
          heroSubtitle:
              'Your selected professional has started the work.',
          accent: _warning,
          showMap: false,
        );
      }
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData ||
          ...
          return _errorView(
            ...
            message:
                'The assigned worker profile could not be loaded.',
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            accent,
            Color.lerp(accent, _secondary, 0.48)!,
          ],
        ),
  ...
  Widget _nearestWorkerFinder(
    ...
    return FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
      future: FirebaseFirestore.instance
          ...
          .where(
            'skill',
            isEqualTo: requestData['category'],
          )
          .limit(1)
          .get(),
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return _errorView(
            ...
            message:
                'We could not search for nearby workers right now.',
            includeTopPadding: false,
          );
        }
  ...
  Future<void> _sendRequestToWorker(
    String workerId,
  ) async {
    ...
    try {
      ...
      _showMessage(
        'Request sent to worker successfully.',
      );
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _openChat({
    ...
    try {
      ...
      String? chatId =
          requestData['chatId']?.toString().trim();
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => ChatDetailScreen(
            chatId: chatId!,
            workerName:
                _fallback(worker['name'], 'Worker'),
            workerSkill:
                _fallback(worker['skill'], 'Unknown'),
          ),
        ),
      );
    } catch (error) {
